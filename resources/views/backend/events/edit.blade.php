     ...
     <div class="col-md-12">
        <div class="panel panel-primary">
            <div class="panel-heading">
               <h3 class="panel-title"><i class="livicon" data-name="list" data-size="18" data-c="#fff" data-hc="#fff" data-loop="true"></i> {{ trans('backend.edit') }} {{ trans('backend.events') }}</h3>
            </div>
            <div class="panel-body">
                <form action="{{ action('EventsController@update', [$events->id]) }}" method="POST" role="form">
                        ...
                        @if($Language->all()->count())
                            ...
                            <div id="group-{{$lang->code}}">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="details_{{$lang->code}}" id="input" class="form-control" rows="5">@if(isset($trans[$lang->code]['details'])){{ $trans[$lang->code]['details'] }}@else{{ old('details_'.$lang->code.'') }}@endif</textarea>
                                </div>

                                <div class="form-group">
                                    <label for="address">{{ trans('backend.address') }} @if($Language->all()->count() > '1') ( {{ $lang->name }} ) @endif</label>
                                    <input type="text" class="form-control" name="address_{{$lang->code}}" value="@if(isset($trans[$lang->code]['address'])){{ $trans[$lang->code]['address'] }}@else{{ old('address_'.$lang->code.'') }}@endif" placeholder="{{ trans('backend.address') }} @if($Language->all()->count() > '1') {{ $lang->name }} @endif">
                                </div>
                            </div>
                                @endforeach
                        @endif
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="website" value="@if(isset($events['website'])){{ $events['website'] }}@else{{ old('website') }}@endif" placeholder="{{ trans('backend.website') }}">
                                </div>
                                ...
                                <div class="form-group">
                                    <label>{{ trans('backend.startdate') }}</label>
                                     <div class="input-group date form_datetime-1" data-date="{{ date('Y-m-d') }}" data-date-format="dd mm yyyy" data-link-field="dtp_input1">
                                       <input size="16" type="text" value="@if(isset($events['startdate'])){{ $events['startdate'] }}@else{{ old('startdate') }}@endif" readonly class="form-control" name="startdate">
                                ...
                                <div class="form-group">
                                    <label>{{ trans('backend.enddate') }}</label>
                                     <div class="input-group date form_datetime-1" data-date="{{ date('Y-m-d') }}" data-date-format="dd mm yyyy" data-link-field="dtp_input1">
                                       <input size="16" type="text" value="@if(isset($events['enddate'])){{ $events['enddate'] }}@else{{ old('enddate') }}@endif" readonly class="form-control" name="enddate">
                                       <span class="input-group-addon">
                                       <span class="glyphicon glyphicon-th"></span>
                                       </span>
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="from" value="@if(isset($events['from'])){{ $events['from'] }}@else{{ old('from') }}@endif" placeholder="{{ trans('backend.from') }}">
                                </div>
                                <div class="form-group">
                                    <label for="to">{{ trans('backend.to') }} @if($Language->all()->count() > '1') ( {{ $lang->name }} ) @endif</label>
                                    <input type="text" class="form-control" name="to" value="@if(isset($events['to'])){{ $events['to'] }}@else{{ old('to') }}@endif" placeholder="{{ trans('backend.to') }} ">
                                </div>
                                ...
                                <div class="form-group" id="menu-group">
                                    ...
                                    <select name="city" class="form-control city_related">
                                        ...
                                        <option class="city_related" value="{{ $city->id }}" @if($events['city']== $city->id) selected @endif>{{ $city->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @endif
                                ...
                                <div class="form-group" id="menu-group">
                                    ...
                                    <select name="area" class="form-control" id="areas">
                                        ...
                                        <option value="{{ $area->id }}" @if ($events['area']== $area->id) selected @endif>{{ $area->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @endif
                    ...
                    <div class="col-md-3 sidbare">
                        ...
                        <div class="form-group">
                            <div class="checkbox">
                                        <label><input name="social_published" type="checkbox" value="1" class="minimal-blue" @if($events->social_published == '1' || old('social_published') == 1) checked @endif> {{ trans('backend.social_published') }}</label>
                                    </div>
                            <div class="checkbox">
                        ...
                        <button type="submit" class="btn btn-block btn-primary">{{ trans('backend.edit') }}</button>
                    </div>
                </form>
            </div>
        </div>
