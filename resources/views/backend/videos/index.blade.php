...
<section class="content-header">
    <h1><i class="livicon" data-name="list" data-size="25" data-c="#418BCA" data-hc="#01BC8C" data-loop="true"></i>{{ trans('backend.list_videos') }}</h1>
    <ol class="breadcrumb">
            <li><a href="/admin"><i class="fa fa-tachometer"></i> {{ trans('backend.dashboard') }}</a></li>
            <li class="active"> {{ trans('backend.list_videos') }}</li>
    </ol>   
</section>
@endsection
        ...
        <div class="portlet box primary">
                ...
                <div class="portlet-body">
                     ...
                     <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            ...
                            <tr>
                                ...
                                <th>
                                    ...
                                    <span class="pull-right sort">
                                        @if(isset($_GET['title']))
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Asc' && isset($_GET['orderby']) && $_GET['orderby'] == 'title') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/videos?title='.$_GET['title'].'&orderby=title&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-asc"></i></a>
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Desc' && isset($_GET['orderby']) && $_GET['orderby'] == 'title') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/videos?title='.$_GET['title'].'&orderby=title&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-desc"></i></a>
                                        @else
                                ...
                                <th>
                                    ...
                                    <span class="pull-right sort">
                                        ...
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Asc' && isset($_GET['orderby']) && $_GET['orderby'] == 'created_at') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/videos?title='.$_GET['created_at'].'&orderby=created_at&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-asc"></i></a>
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Desc' && isset($_GET['orderby']) && $_GET['orderby'] == 'created_at') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/videos?title='.$_GET['created_at'].'&orderby=created_at&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-desctitle"></i></a>
                                        @else
                                ...
                                <th>
                                    ...
                                    <span class="pull-right sort">
                                        ...
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Asc' && isset($_GET['orderby']) && $_GET['orderby'] == 'updated_at') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/videos?title='.$_GET['updated_at'].'&orderby=updated_at&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-asc"></i></a>
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Desc' && isset($_GET['orderby']) && $_GET['orderby'] == 'updated_at') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/videos?title='.$_GET['updated_at'].'&orderby=updated_at&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-desctitle"></i></a>
                                        @else
                                ...
                                <th>
                                    ...
                                    <span class="pull-right sort">
                                        ...
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Asc' && isset($_GET['orderby']) && $_GET['orderby'] == 'published') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/videos?title='.$_GET['published'].'&orderby=published&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-asc"></i></a>
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Desc' && isset($_GET['orderby']) && $_GET['orderby'] == 'published') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/videos?title='.$_GET['published'].'&orderby=published&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-desctitle"></i></a>
                                        @else
                        ...
                        @foreach($videos as $item)
                            <tr>
                                ...
                                <td>
                                    @if($item->published == '1')
                                       <a href="{{action('VideoAlbumController@single_status', ['2', $item->id])}}" class="label label-sm label-success"><span class="glyphicon glyphicon-ok"></span> {{ trans('backend.published') }}</a>
                                    @elseif($item->published == '2')
                                        <a href="{{action('VideoAlbumController@single_status', ['1', $item->id])}}" class="label label-sm label-warning"><span class="glyphicon glyphicon-ban-circle"></span> {{ trans('backend.unpublished') }}</a>
                                    @endif
                                </td>
                                -->
                                <td class="">
                                <a href="{{action('VideoAlbumController@edit', [$item->id])}}"><i class="fa  fa-edit"></i> {{ trans('backend.edit') }}</a>
                        ...
                        <div class="text-center">
                            ...
                            <h1>{{ trans('backend.no_results_found') }}
                             <a href="/{{Lang::getlocale()}}/admin/videos">{{ trans('backend.back') }}</a></h1>
                            @elseif(Request::is(''.Lang::getlocale().'/admin/videos'))
                            <h1>{{ trans('backend.no_data_added_before') }}
                             <a href="/{{Lang::getlocale()}}/admin/videos/create">{{ trans('backend.create_content') }}</a></h1>
                            @endif
                         </div>
                        @endif
                    @if($videos->count())
                    ...
                    <div class="form-group bulk">
                        ...
                        <!--
                        <a class="btn btn-success btn-large" onclick="submitForm('{{action('VideoAlbumController@bulk_status', ['1'])}}')"><span class="glyphicon glyphicon-ok"></span> {{ trans('backend.bulk_publish') }}</a>
                        <a class="btn btn-warning btn-large" onclick="submitForm('{{action('VideoAlbumController@bulk_status', ['2'])}}')"><span class="glyphicon glyphicon-ban-circle"></span> {{ trans('backend.bulk_unpublish') }}</a>
                        -->
                    </div>
                   </form>
                   <div class="table-footer">
                         <div class="count"> <i class="fa fa-folder-o"></i> {{ trans('backend.total') }} : {{ $videos->total() }} {{ trans('backend.videos') }} </div>
                        <div class="pagination-area"> {!! $videos->render() !!} </div>
                    </div>
                @endif
                </div>
            </div>
...
<script type="text/javascript">
    function bulkconfirmDelete() {
        ...
        http.open("POST", "{{action('VideoAlbumController@bulk_destroy_confirm')}}", true);
