...
<section class="content-header">
    <h1><i class="livicon" data-name="list" data-size="25" data-c="#418BCA" data-hc="#01BC8C" data-loop="true"></i>{{ trans('backend.edit') }} {{ trans('backend.videos') }}</h1>
    <ol class="breadcrumb">
      <li><a href="/{{ Lang::getlocale() }}/admin"> {{ trans('backend.dashboard') }}</a></li>
      <li><a href="/{{ Lang::getlocale() }}/admin/videos"> {{ trans('backend.list_videos') }}</a></li>
      <li class="active"> {{ trans('backend.edit') }} {{ trans('backend.videos') }}</li>
    </ol>   
</section>
@endsection
     ...
     <div class="col-md-12">
        <div class="panel panel-primary">
            <div class="panel-heading">
                        <h3 class="panel-title"><i class="livicon" data-name="list" data-size="18" data-c="#fff" data-hc="#fff" data-loop="true"></i>{{ trans('backend.create') }} {{ trans('backend.videos') }}</h3>
            </div>
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-12">
                        <form action="{{ action('VideoAlbumController@update', [$videos->id]) }}" method="POST" role="form">
                            ...
                            @if($Language->all()->count())
                                ...
                                <div id="group-{{$lang->code}}">
                                    ...
                                    <div class="form-group">
                                        <label for="subject">{{ trans('backend.brief') }} @if($Language->all()->count() > '1') ( {{ $lang->name }} ) @endif</label>
                                        <textarea name="subject_{{$lang->code}}" id="input" class="form-control" rows="5">@if(isset($trans[$lang->code]['subject'])){{ $trans[$lang->code]['subject'] }}@else{{ old('subject_'.$lang->code.'') }}@endif</textarea>
                                    </div>
                                </div>
                                @endforeach
                            @endif
