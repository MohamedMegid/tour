...
<section class="content-header">
    <h1><i class="livicon" data-name="comment" data-size="25" data-c="#418BCA" data-hc="#01BC8C" data-loop="true"></i>{{ trans('backend.comments') }}</h1>
    <ol class="breadcrumb">
            <li><a href="/admin"><i class="fa fa-tachometer"></i> {{ trans('backend.dashboard') }}</a></li>
            <li class="active"> {{ trans('backend.list_comments') }}</li>
    </ol>   
</section>
@endsection
...
@include('backend.comments.confirm-delete')
<!-- end include single delete confirmation model -->  

<!-- Include bulk delete confirmation model -->
@include('backend.comments.bulk-confirm-delete')
        ...
        <div class="portlet box primary">
                ...
                <div class="portlet-body">
                     ...
                     <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        ...
                        @foreach($comments as $item)
                            <tr>
                                       <td>
                                        ...
                                        <td>
                                        	<?php
                                                ...
                                                if (strlen($comment) > $limit)
                                                    $comment = substr($comment, 0, strrpos(substr($comment, 0, $limit), ' ')) . '...';
                                            ?>
                                        	{{$comment}}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                        @if($item->site)
                                            @if($item->site->TransLangCheck)
                                            {{ $item->site->TransLangCheck->title }}
                                            @elseif($item->site->TransWithOtherLang)
                                            {{ $item->site->TransWithOtherLang->title }}
                                            @endif
                                        @endif
                                        </td>
                                        <td>
                                            @if($item->author)
                                                {{ $item->author->name }}
                                            @else
                                                {{trans('backend.visitor')}}
                                            @endif
                                        </td>
                                        <td>
                                         @if($item->published == '1')
	                                       <a href="{{action('CommentsController@single_status', ['2',$item->id])}}" class="label label-sm label-success"><span class="glyphicon glyphicon-ok"></span> {{ trans('backend.published') }}</a>
	                                    @elseif($item->published == '2')
	                                        <a href="{{action('CommentsController@single_status', ['1',$item->id])}}" class="label label-sm label-warning"><span class="glyphicon glyphicon-ban-circle"></span> {{ trans('backend.unpublished') }}</a>
	                                    @endif
                                        </td>
                                        <td>{{ $item->created_at }}</td>
                                        <td>{{ $item->updated_at }}</td>
                                        <td>
                                          <a href="{{action('CommentsController@edit', $item->id)}}"><i class="fa  fa-edit"></i> {{ trans('backend.edit') }}</a>
                                          <a onclick="confirmDelete(this)" data-toggle="modal" data-href="#full-width" data-id="{{ $item->id }}" data-title="{{ $item->comment }}" href="#full-width"><i class="fa fa-trash-o"></i> {{ trans('backend.delete') }}</a>
                                    </tr>
                        @endforeach
                        @else
                        <div class="text-center">
                            ...
                            <h1>{{ trans('backend.no_results_found') }}
                             <a href="/{{Lang::getlocale()}}/admin/comments">{{ trans('backend.back') }}</a></h1>
                            @elseif(Request::is(''.Lang::getlocale().'/admin/comments'))
                    ...
                    <div class="form-group bulk">
                        ...
                        <a class="btn btn-success btn-large" onclick="submitForm('{{action('CommentsController@bulk_status', 1)}}')"><span class="glyphicon glyphicon-ok"></span> {{ trans('backend.bulk_publish') }}</a>

                        <a class="btn btn-warning btn-large" onclick="submitForm('{{action('CommentsController@bulk_status', 2)}}')"><span class="glyphicon glyphicon-ban-circle"></span> {{ trans('backend.bulk_unpublish') }}</a>
                    </div>
                   </form>
                   <div class="table-footer">
                         <div class="count"> <i class="fa fa-folder-o"></i> {{ trans('backend.total') }} : {{ $comments->total() }} {{ trans('backend.comment') }} </div>
                        <div class="pagination-area"> {!! $comments->render() !!} </div>
                    </div>
                @endif
                </div>
            </div>
...
<script type="text/javascript">
    function bulkconfirmDelete() {
        ...
        http.open("POST", "{{action('CommentsController@bulk_destroy_confirm')}}", true);
