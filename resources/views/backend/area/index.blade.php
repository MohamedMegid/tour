...
<section class="content-header">
    <h1><i class="livicon" data-name="list" data-size="25" data-c="#418BCA" data-hc="#01BC8C" data-loop="true"></i>{{ trans('backend.list_area') }}</h1>
    <ol class="breadcrumb">
            <li><a href="/admin"><i class="fa fa-tachometer"></i> {{ trans('backend.dashboard') }}</a></li>
            <li class="active"> {{ trans('backend.list_area') }}</li>
    </ol>   
</section>
@endsection
        ...
        <div class="portlet box primary">
                ...
                <div class="portlet-body">
                     ...
                     <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            ...
                            <tr>
                                ...
                                <th>
                                    ...
                                    <span class="pull-right sort">
                                        @if(isset($_GET['name']))
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Asc' && isset($_GET['orderby']) && $_GET['orderby'] == 'name') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/area?name='.$_GET['name'].'&orderby=name&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-asc"></i></a>
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Desc' && isset($_GET['orderby']) && $_GET['orderby'] == 'name') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/area?name='.$_GET['name'].'&orderby=name&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-desc"></i></a>
                                        @else
                                ...
                                <th>
                                    ...
                                    <span class="pull-right sort">
                                        ...
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Asc' && isset($_GET['orderby']) && $_GET['orderby'] == 'created_at') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/area?name='.$_GET['created_at'].'&orderby=created_at&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-asc"></i></a>
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Desc' && isset($_GET['orderby']) && $_GET['orderby'] == 'created_at') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/area?name='.$_GET['created_at'].'&orderby=created_at&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-desctitle"></i></a>
                                        @else
                                ...
                                <th>
                                    ...
                                    <span class="pull-right sort">
                                        ...
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Asc' && isset($_GET['orderby']) && $_GET['orderby'] == 'updated_at') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/area?name='.$_GET['updated_at'].'&orderby=updated_at&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-asc"></i></a>
                                        <a @if(isset($_GET['sort']) && $_GET['sort'] == 'Desc' && isset($_GET['orderby']) && $_GET['orderby'] == 'updated_at') class="active" @endif href="{{url(''.Lang::getlocale().'/admin/area?name='.$_GET['updated_at'].'&orderby=updated_at&sort=Asc&page='.isset($_GET['page']).'')}}"><i class="fa fa-sort-desctitle"></i></a>
                                        @else
                        ...
                        @foreach($items as $item)
                            <tr>
                                ...
                                -->
                                <td class="">
                                <a href="{{action('AreaController@edit', [$item->id])}}"><i class="fa  fa-edit"></i> {{ trans('backend.edit') }}</a>
                        ...
                        <div class="text-center">
                            ...
                            <h1>{{ trans('backend.no_results_found') }}
                             <a href="/{{Lang::getlocale()}}/admin/area">{{ trans('backend.back') }}</a></h1>
                            @elseif(Request::is(''.Lang::getlocale().'/admin/area'))
                            <h1>{{ trans('backend.no_data_added_before') }}
                             <a href="/{{Lang::getlocale()}}/admin/area/create">{{ trans('backend.create_content') }}</a></h1>
                            @endif
                         </div>
                        @endif
                    @if($items->count())
                   ...
                   <div class="table-footer">
                         <div class="count"> <i class="fa fa-folder-o"></i> {{ trans('backend.total') }} : {{ $items->total() }} {{ trans('backend.area') }} </div>
...
<script type="text/javascript">
    function bulkconfirmDelete() {
        ...
        http.open("POST", "{{action('AreaController@bulk_destroy_confirm')}}", true);
