...
<section class="content-header">
    <h1><i class="livicon" data-name="phone" data-size="30" data-c="#418BCA" data-hc="#01BC8C" data-loop="true"></i>
      {{ trans('backend.Complains Sections')}}</h1>
    <ol class="breadcrumb">
          <li><a href="/{{Lang::getlocale()}}/admin"><i class="fa fa-tachometer"></i> {{ trans('backend.dashboard')}}</a></li>
          <li class="active"><i class="fa fa-flag-o"></i> {{ trans('backend.List Complains Sections')}}</li>
    </ol>   
</section>
@endsection
...
@section('content')
<!-- Include filter -->
@include('backend.complainsections.filter')
<!-- end include filter -->  

<!-- Include single delete confirmation model -->
@include('backend.complainsections.confirm-delete')
<!-- end include single delete confirmation model -->  

<!-- Include bulk delete confirmation model -->
@include('backend.complainsections.bulk-confirm-delete')
                  ...
                  <div class="portlet-title">
                      <div class="caption">
                        <i class="livicon" data-name="responsive" data-size="16" data-loop="true" data-c="#fff" data-hc="white"></i>
                         {{ trans('backend.List Complains Sections')}}
                      </div>
                   </div>
                   ...
                   <div class="portlet-body">
                          <div class="table-responsive">
                                 ...
                                 <table class="table table-bordered">
                                    <thead class="flip-content">
                                        <tr>
                                          ...
                                          <th>{{ trans('backend.last_update')}}</th>
                                      ...
                                      @foreach($items as $item)
                                        <tr>
                                          <td><input type="checkbox" name="ids[]" class="check_list" value="{{$item->id}}"></td>
                                          <td> {{ $item->title }} </td>
                                          <td> {{ $item->created_at }} </td>
                                          <td> {{ $item->updated_at }} </td>
                                          <td>
                                            <a href="{{action('ComplainSections@edit', [$item->id])}}"><i class="fa  fa-edit"></i> {{ trans('backend.edit') }}</a>
                                ...
                                @endif
                                 <div class="count"> <i class="fa fa-folder-o"></i> {{ trans('backend.total')}} : {{ $items->total() }} {{ trans('backend.Complains Sections')}} </div>
...
<script type="text/javascript">
    function bulkconfirmDelete() {
        ...
        http.open("POST", "{{action('ComplainSections@bulk_destroy_confirm')}}", true);
